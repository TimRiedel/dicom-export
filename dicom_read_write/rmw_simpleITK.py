...
def create_dummy_mask(rows, cols, as_string=True):
    if as_string:
        overlay_size = rows*cols
        bits = "1"*(int(overlay_size))
        return bits
...
series_tag_values = [
                    ...
                    # set own values test
                    ("0010|0010", f"Peter Pan {time.asctime()}"),  # Patient Name (overwrite)
                    ("0010|0040", "M"),  # Patient Sex (new tag)
